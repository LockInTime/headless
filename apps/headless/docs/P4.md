...
required fields, byte and numeric bounds, enum values, and array limits come
from Swift definitions that also run before the command-specific protocol
validators. `sdk/protocol-schema.json` is a checked-in golden artifact; the
protocol suite fails if it drifts from the executable output.
...
to an existing host, inherits a live standard-input owner pipe, and shuts down
when that pipe closes. An SDK must never stop a shared host implicitly.
