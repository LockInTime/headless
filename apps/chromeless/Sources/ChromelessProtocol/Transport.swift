...
public enum LocalTransportError: Error, CustomStringConvertible {
    ...
    public var description: String {
        ...
        case .connectionFailed: return "Chromeless host is not running"
        case .timedOut: return "Chromeless host did not respond before the deadline"
        case .connectionClosed: return "Chromeless host closed the connection"
        case .messageTooLarge: return "Chromeless host message exceeded the size limit"
        case .alreadyRunning: return "Another Chromeless host is already using the local socket"
        }
    }
}
...
public enum LocalRuntime {
    public static var directoryURL: URL {
        URL(fileURLWithPath: "/tmp", isDirectory: true)
            .appendingPathComponent("chromeless-\(currentUserID())", isDirectory: true)
    }

    public static var socketURL: URL {
        if let override = ProcessInfo.processInfo.environment["CHROMELESS_SOCKET"], override.hasPrefix("/") {
...
public final class LocalSocketServer: @unchecked Sendable {
    ...
    public let socketPath: String
    private let queue = DispatchQueue(label: "com.chromeless.socket-server", qos: .userInitiated)
    ...
    private let clientQueue = DispatchQueue(
        label: "com.chromeless.socket-clients", qos: .userInitiated, attributes: .concurrent
    )
    // Browser hosts mutate session state, so ordinary requests retain their
    // original one-at-a-time execution semantics.
    private let requestQueue = DispatchQueue(label: "com.chromeless.socket-requests", qos: .userInitiated)
...
private func readLine(from fd: Int32) throws -> Data {
    ...
    while result.count <= chromelessMaximumMessageBytes {
