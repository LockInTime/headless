...
public let chromelessProtocolVersion = "0.3"
public let chromelessMaximumMessageBytes = 1_048_576
...
public struct CommandRequest: Codable, Equatable, Sendable {
    ...
    public init(
        ...
        version: String = chromelessProtocolVersion,
        command: CommandName,
        session: String? = nil,
    ...
    public func validate() throws {
        guard version == chromelessProtocolVersion else {
...
public struct CommandResponse: Codable, Equatable, Sendable {
    ...
    public static func success(id: String, result: JSONValue = .object([:])) -> CommandResponse {
        CommandResponse(id: id, version: chromelessProtocolVersion, ok: true, result: result, error: nil)
    }

    public static func failure(id: String, code: String, message: String, suggestion: String? = nil) -> CommandResponse {
        CommandResponse(
            id: id,
            version: chromelessProtocolVersion,
            ok: false,
            result: nil,
...
public enum ProtocolCodec {
    ...
    public static func encodeLine<T: Encodable>(_ value: T) throws -> Data {
        ...
        data.append(0x0A)
        guard data.count <= chromelessMaximumMessageBytes else { throw CodecError.messageTooLarge }
        return data
    }

    public static func decodeLine<T: Decodable>(_ type: T.Type, from data: Data) throws -> T {
        guard data.count <= chromelessMaximumMessageBytes else {
