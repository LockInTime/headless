...
struct ProtocolTests {
    ...
    static func messageSizeLimit() throws {
        let exactPayload = String(repeating: "a", count: chromelessMaximumMessageBytes - 3)
        let exactLine = try ProtocolCodec.encodeLine(exactPayload)
        try expect(exactLine.count == chromelessMaximumMessageBytes, "framed message should fit the exact limit")
        try expect(
        ...
        try expectThrows("encoding beyond the framed limit should be rejected") {
            _ = try ProtocolCodec.encodeLine(String(repeating: "a", count: chromelessMaximumMessageBytes - 2))
        }
        let data = Data(repeating: 0x61, count: chromelessMaximumMessageBytes + 1)
    ...
    static func artifactStoreRoundTrip() throws {
        ...
        defer { try? FileManager.default.removeItem(atPath: root) }
        let store = try ArtifactStore(environment: ["CHROMELESS_ARTIFACT_DIR": root])
        ...
        try expectThrows("artifact root symlinks should be rejected") {
            _ = try ArtifactStore(environment: ["CHROMELESS_ARTIFACT_DIR": symlinkRoot])
        }
    }
    ...
    static func serverRejectsSocketOutsidePrivateDirectory() throws {
        let server = LocalSocketServer(socketPath: "/tmp/chromeless-outside-\(UUID().uuidString).sock")
