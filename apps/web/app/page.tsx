    ...
    <main className="site-shell">
        ...
        <div id="top" className="container hero-grid">
          <div className="hero-content">
            ...
            <p className="hero-copy">
              ...
              surface, so an agent can navigate, test, record, and explain
              without screen coordinates or a brittle script stack.
            </p>
      ...
      <section className="benchmark" aria-labelledby="benchmark-title">
        <div className="container benchmark-header">
          <div>
            <div className="section-label">{benchmark.sectionLabel}</div>
            <h2 id="benchmark-title">
              {benchmark.headline}
              <br />
              <em>Measured, not claimed.</em>
            </h2>
          </div>
          <p>{benchmark.summary}</p>
        </div>
        <div
          className="container benchmark-proof"
          aria-label="Headless comparisons with Selenium and Puppeteer across tokens and CPU time"
        >
          {benchmark.proofs.map((proof) => (
            <article key={`${proof.metric}-${proof.against}`}>
              <span>
                {proof.metric} · vs {proof.against}
              </span>
              <strong>
                {proof.value}
                <small>%</small>
        ...
        <div className="container benchmark-grid-2">
          <div className="benchmark-chart-panel">
            <div>
              <span>Agent token footprint / lower is better</span>
              <p>Headless warm = 1×</p>
            </div>
            <BenchmarkChart data={benchmark.workflows} />
          </div>
          <div className="benchmark-chart-panel">
            <div>
              <span>Wall time vs CPU time per run</span>
              <p>The gap is waiting, not computing</p>
            </div>
            <EfficiencyChart
              data={benchmark.workflows.map((workflow) => ({
                ...workflow,
                label: workflow.variant || workflow.name,
              }))}
            />
          </div>
        </div>
        <div className="container benchmark-table-wrap">
          <table className="benchmark-table">
            ...
            <tbody>
              {benchmark.workflows.map((workflow) => (
                <tr
                  className={
                    workflow.case === "headless-warm"
                      ? "benchmark-highlight"
                      : ""
                  }
                  key={workflow.case}
                >
                  <td>
                    <b>{workflow.name}</b>
                    {workflow.variant && <span>{workflow.variant}</span>}
                  </td>
                  <td>{workflow.formatted.tokens}</td>
                  <td>{workflow.formatted.wallTime}</td>
                  <td>{workflow.formatted.cpuTime}</td>
                  <td>{workflow.formatted.memory}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <div className="container benchmark-footnote">
          <p>
            <span className="benchmark-definition">
              <b>Warm</b> starts with the Headless host and session ready.{" "}
              <b>Cold</b> includes starting both.
            </span>
            <span>
              Estimated tokens measure workflow source size, not billed LLM
              tokens. Point-in-time snapshot; repeat before comparing a change.
            </span>
          </p>
          <a
            className="text-link"
            href="https://github.com/LockInTime/headless/blob/main/apps/headless/docs/BENCHMARK.md"
          >
            Read the method <LinkGlyph kind="external" />
          </a>
        </div>
      </section>
      ...
      <section id="workflow" className="section container workflow">
        ...
        <div className="section-heading">
          <p>From intent to evidence—</p>
          <h2>without touching the desktop.</h2>
        </div>
        <div className="workflow-layout">
          ...
          <div
            className="terminal"
            role="img"
            aria-label="Example Headless QA command sequence"
          >
            <div className="terminal-head">
              <span />
              <span />
              <span />
              <p>hermes-vm — headless</p>
              <b>secure local session</b>
            </div>
            <div className="terminal-body">
              <p>
                <i>$</i> headless start <em>✓ ready</em>
              </p>
              <p>
                <i>$</i> headless session create qa
              </p>
              <p>
                <i>$</i> headless --session qa visit
                localhost:3000/designers/dashboard
              </p>
              <p>
                <i>$</i> headless --session qa record start --fps 10
              </p>
              <p>
                <i>$</i> headless --session qa click --role button --name
                Continue
              </p>
              <p>
                <i>$</i> headless --session qa report create --output
                pr-report.json <em>✓ evidence ready</em>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="container capability-layout">
          <div className="capability-intro">
            ...
            <h2>
              ...
              <br />
              the agent can <em>look closer.</em>
            </h2>
            <p>
              Deep diagnostics are available on demand, not forced into every
              interaction.
            </p>
          </div>
          <div className="capability-list">
            {capabilities.map(([title, description, safe], index) => (
              <article
                key={title}
                className={
                  safe ? "capability-row capability-row-safe" : "capability-row"
                }
              >
                <span>0{index + 1}</span>
                <div>
        ...
        <div className="container closing-inner">
          <div className="closing-panel">
            ...
            <div className="closing-action">
              <p>
                One CLI protocol. The same commands over MCP. Purpose-built for
                the last mile of software QA.
              </p>
              <a
      ...
      <footer className="wordmark-footer">
        <div className="container footer-meta">
          <a className="text-link" href="/docs">
            DOCUMENTATION <LinkGlyph kind="external" />
          </a>
        </div>
        <div className="footer-wordmark" aria-label="headless">
          headless
        </div>
      </footer>
    </main>
