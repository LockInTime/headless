...
function disposeThree(t: ThreeBag, container: HTMLElement) {
...
export function PixelBlast({
  ...
  useEffect(() => {
    ...
    return () => {
      // Skip dispose when this run created a fresh WebGL context — the next
      // effect body (or the unmount effect) owns teardown.
  ...
  // Always dispose on unmount (covers the mustReinit skip path above).
  useEffect(() => {
    const container = containerRef.current;
    return () => {
