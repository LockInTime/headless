...
| **Artifacts**                   | Bare validated names, `O_EXCL` creation at `0600` inside a `0700` root, never overwritten. Path traversal or reading outside the store is a vulnerability. Ingest of operator files is a local CLI path, never a protocol or MCP command. |
