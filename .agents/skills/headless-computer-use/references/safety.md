...
To attach a file after that confirmation, ingest it with
`headless artifacts add SOURCE --name FILE` and then
`headless upload --role textbox --name NAME --artifact FILE` (or `upload @eN
--artifact FILE`). Upload accepts an artifact basename only — never a
home-directory path. Downloads remain denied. File bytes never appear on the
protocol socket. macOS WebKit returns `UNSUPPORTED_CAPABILITY` until a native
attach path exists.
