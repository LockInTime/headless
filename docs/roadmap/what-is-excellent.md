...
the _idea_ is right and the implementation expresses it well. They are the
...
preserved. (Known implementation bugs _inside_ these areas are still fixable —
...
prompt-injected or confused agent _cannot_ violate them.
...
- **Artifact discipline.** Names validated (no paths, no leading dot, charset,
  ...
  (`Artifacts.swift`).
- **Sensitive values gated twice.** Cookie/storage _values_ need both
...
- E2E suites assert _artifact truth_, not just exit codes: `file(1)` magic,
...
an _agent-surface_ proxy — explicitly "not billed LLM tokens"), publishes the
dimensions it _loses_ (Puppeteer is faster on wall time), refuses to claim
...
refreshed; the caveat culture may not be dropped. The website now derives its
claims from the generated result artifact and carries the same point-in-time
warning.
...
**Preserve:** never ship a silent partial emulation. (Existing _accidental_
...
**one tool** whose input is the CLI argv. The MCP surface _is_ the CLI
